        ...
        <ul className="lang__menu">
          {LANGUAGES.map((l) => (
            <li key={l.code}>
              <button
                type="button"
                className={`lang__option ${l.code === current.code ? 'active' : ''}`}
                onClick={() => {
                  onChange(l.code)
                  setOpen(false)
                }}
...
function OrderOptionRow({ option, text }) {
  return (
    <li>
      <a className="option" href={option.href} target={option.href.startsWith('http') ? '_blank' : undefined} rel={option.href.startsWith('http') ? 'noopener noreferrer' : undefined}>
      ...
      {option.divideAfter && <div className="option-divider" />}
    </li>
  )
}
